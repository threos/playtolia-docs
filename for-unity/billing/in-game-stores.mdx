...
**EventBus** component offers a fast, non-blocking, and safe way to register callbacks to consume results of async flows.
...
```c#
StoreItem item = PlaytoliaStore.GetItemBySku("com.mygame.coinpack_500");
PlaytoliaStore.BeginPurchaseFlow(item, handle); // notice the extra 'handle' parameter
```
